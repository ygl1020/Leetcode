#编辑距离求每次删除一个元素,两个数组都可以删除,需要几次w1才等于w2
def minDistance(self, word1: str, word2: str) -> int:
        ...
        当word1[j-1] != word2[i-1]， 我们可以删除word1的最后一个元素,删除word2的最后一个元素和删除words1和word2的最后一个元素-->dp[i][j] = min(dp[i-1][j]+1, dp[i][j-1]+1, dp[i-1][j-1]+2)
