...
def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        ...
        这题和300再一个数组中找到不连续的最长公共子数组以及718在两个数组中寻找连续的公共子数组思路类似.这题是之前这两题的结合版.我们需要额外考虑的是当
        text2[j-1] != text1[i-1]的情况,举例 'abc' 和'ace',当第一个字符串中的c和第二个字符串中的最后一个e不相同时,我们考虑两种情况,1)abc和ac中找最长
        2）ab和ace中找最长.那么对应的dp公式就是dp[i][j] = max(dp[i-1][j], dp[i][j-1])
        """
